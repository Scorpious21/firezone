...
try{ Object.assign(settings, JSON.parse(localStorage.getItem('fz_settings')||'{}')); }catch(e){}
...
export const Audio = {
  ...
  applyVolumes(){ if(!this.ctx) return; this.master.gain.value = settings.master; this.sfxGain.gain.value = settings.sfx; },
  ...
  enemyShot(v=.35){ this.noise(.16, 2400, 260, v, 'lowpass', 1); this.tone(140, 45, .1, v*.6, 'sine'); },
  hit(head){ if(head){ this.tone(1400,900,.09,.35,'square'); this.tone(2100,1500,.06,.2,'sine',.02); } else this.tone(880,620,.07,.28,'square'); },
...
export const Haptics = {
  ...
  hurt(){ this.buzz([40,25,55]); }, death(){ this.buzz([110,55,160,180]); }
};

export function applySettingsToDOM(){
  document.documentElement.style.setProperty('--chc', settings.crosshairColor);
  document.documentElement.classList.toggle('lefty', !!settings.lefty);
